...
# 0x0	/* D-MACHINE VERSION BYTE */
# 0x1	/* MODE BYTE */
# 0x2	/* GAME ID */
# 0x4	/* END OF GAME FILE PRELOAD (WORD PTR) */
# 0x6	/* EXECUTION START ADDR	    (WORD PTR "ODD") */
# 0x8	/* BEGINNING OF PURE CODE   (WORD PTR) */
# 0xA	/* GLOBAL TABLE 	    (WORD PTR) */
# 0xC	/* 6 BYTE SERIAL NUMBER (DATE) */
# 0x12	/* LENGTH OF GAME FILE, IN WORDS */
# 0x14	/* CHECKSUM OF GAME FILE, EXCLUDING HEADER */
# 0x16	/* INTERPRETER ID, LETTER & NUMBER */

# 0x40	/* LENGTH OF HEADER, PROGRAM FILE */

# 0x0	/* length of image file, in words */
# 0x2	/* end of preload, word ptr */
# 0x4	/* checksum of image file */
# 0x6	/* total number of blocksets in the file */
# 0x7	/* total number of icons in the file */

# 0x8	/* LENGTH OF HEADER, IMAGE FILE */

# 0x1	/* padding between code and image files, in words */
...
def globalsloc():
    ...
    if not globals_location:
        globals_location = zcode.memory.getword(0xa)
...
def statmembase():
    ...
    if not static_memory:
        static_memory = zcode.memory.getword(0x8)
